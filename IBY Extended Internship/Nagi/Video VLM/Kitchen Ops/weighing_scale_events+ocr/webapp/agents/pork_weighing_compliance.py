...
# Import shared tuning constants — auto_roi.py is the single source of truth for these.
import sys as _sys, os as _os
_sys.path.insert(0, _os.path.join(_os.path.dirname(__file__), ".."))
from auto_roi import CIRCLE_RADIUS_MARGIN
...
RED CIRCLE VISUAL PROMPTS:
- A bright RED CIRCLE has been drawn over the location(s) where the digital scale display is expected to be
- Use these circles as visual cues to focus your OCR reading; the weight value will be inside or very near these regions
...
RED CIRCLE VISUAL PROMPTS:
- A bright RED CIRCLE has been drawn over the location(s) where the digital scale display is expected to be
- Use these circles as visual cues to focus your verification; the weight value will be inside or very near these regions
...
class PorkWeighingPipeline:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        if config.display_circles and self.roi:
            ...
            for d in config.display_circles:
                ...
                cx_full = (dx1 + dx2) / 2
                cy_full = (dy1 + dy2) / 2
                # Convert from full-frame pixel coords to raw-crop-space pixel coords
                cx = int(cx_full - rx1)
                cy = int(cy_full - ry1)
                r = int(max(dx2 - dx1, dy2 - dy1) / 2 * (1.0 + CIRCLE_RADIUS_MARGIN))
                self.display_circles.append((cx, cy, r))
            # Store raw crop dimensions so draw_display_circles can scale to the
            # upscaled frame correctly (scale_x = upscaled_w / raw_crop_w).
            self._display_circles_ref_w = rx2 - rx1
            self._display_circles_ref_h = ry2 - ry1
    ...
    _DISPLAY_DETECT_PROMPT = (
        ...
        "Your task is to locate every digital readout panel (LCD/LED display) of the weighing scales "
        "visible in this image. These are usually small white or silver electronic scales. "
        "Each display is a small rectangular face showing digital numeric weight values. "
        "From this high-angle perspective, the displays are typically located on the top surface or front edge of the scale base. "
        "There may be multiple scales (e.g., side-by-side or stacked). "
        "For EACH scale display panel found, return a bounding box drawn tightly around just the LCD/LED display area - "
        "do not include the weighing platform, buttons, or the whole scale body. "
        ...
        "If no digital display is clearly visible, return {\"displays\": []}."
    )
    ...
    def detect_display_circles(self, video_path: str) -> None:
        ...
        # Sample 3 frames spread across the first 20% of the video
        sample_indices = [int(total * f) for f in (0.05, 0.10, 0.15)]
        sample_indices = [min(max(0, idx), total - 1) for idx in sample_indices]

        # Sequence: rotate → crop → upscale → sharpen; VLM sees the same quality as analysis
        ...
        for idx in sample_indices:
            ...
            if not ret:
                continue
            prepared = self.apply_clahe(self.upscale_frame(self.crop_frame(self.rotate_frame(frame))))
            if prepared.size > 0:
        ...
        for crop in prepared_frames:
            # Encode as JPEG (smaller payload — detail:high for display panel accuracy)
        ...
        # Convert bounding boxes → (cx, cy, radius) using the first prepared (upscaled+sharpened) frame dimensions
        ref_h, ref_w = prepared_frames[0].shape[:2]
        ...
        for x1n, y1n, x2n, y2n in all_boxes:
            ...
            bw = (x2n - x1n) * ref_w
            bh = (y2n - y1n) * ref_h
            radius = int(max(bw, bh) / 2 * (1.0 + CIRCLE_RADIUS_MARGIN))
    ...
    def draw_display_circles(self, frame: np.ndarray) -> np.ndarray:
        ...
        Coordinates are in the space of the crop *before* upscaling, so they are
        scaled proportionally to match the upscaled frame dimensions.
        """
        if not self.display_circles:
            return frame

        annotated = frame.copy()
        fh, fw = frame.shape[:2]

        # display_circles were recorded on the raw crop; upscale_frame may have changed
        # the size. We store the reference dimensions alongside so we can scale correctly.
        ref_w = getattr(self, "_display_circles_ref_w", fw)
        ref_h = getattr(self, "_display_circles_ref_h", fh)
        ...
        for cx, cy, radius in self.display_circles:
            ...
            scaled_r  = int(radius * max(scale_x, scale_y))
            cv2.circle(annotated, (scaled_cx, scaled_cy), scaled_r, (0, 0, 255), 3)
    ...
    def _ensure_sr_model(self) -> bool:
        ...
        # Weight search order: local weights/ dir first, then ~/.cache/realesrgan/
        ...
        if weights_path is None:
            ...
            try:
                ...
                logger.warning(f"Real-ESRGAN weight download failed ({exc}); digit SR skipped.")
                return False
    ...
    def _apply_digit_sr(self, frame: np.ndarray) -> np.ndarray:
        ...
        - No display circles have been detected
        - Any per-patch SR call fails (that patch is left as-is)
        """
        if not self.display_circles or not self._ensure_sr_model():
            return frame
    ...
    def prepare_frame_for_analysis(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.config.enable_cropping:
            ...
            return self.draw_display_circles(enhanced)
        else:
    ...
    def save_clahe_preview_frames(self, video_path: str) -> None:
        ...
        for pos in positions:
            ...
            processed = self.prepare_frame_for_analysis(rotated)
    ...
    def run_phase1(self, video_path: str) -> List[Detection]:
        ...
        # Detect scale display locations once; results are drawn on every frame via
        # prepare_frame_for_analysis → draw_display_circles.
        if self.display_circles is None:
            try:
                self.detect_display_circles(video_path)
            except Exception as _dc_err:
                logger.warning(f"Display circle detection failed (non-fatal): {_dc_err}")
                self.display_circles = []

        # Save preview frames after circles are resolved (whether pre-detected or
        # freshly detected). This ensures the dashboard always gets annotated frames.
        try:
            self.save_clahe_preview_frames(video_path)
        except Exception as _prev_err:
            logger.warning(f"save_clahe_preview_frames failed (non-fatal): {_prev_err}")
