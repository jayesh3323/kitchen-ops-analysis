...
| I2 | Sequential ROI & Display VLM Pipeline | `auto_roi.py` | Two-step sequential flow (ROI first, then Displays using ROI as context). Dramatically reduces false-positive "food circles". | I3 | Red Circle Visual Prompting | All 5 agents | **Red Circle Visual Prompting** — draw `cv2.circle()` on the exact region the VLM must attend to (red outperforms all other colors/shapes). 72–128% relative improvement on localization (keypoint: 42.2%→7s% PCP) | ✅ Working |
