...
| G1 | **JSON schema enforcement** — pass `response_format={"type":"json_schema",...}` to GPT; `response_schema` to Gemini | OpenAI + Google API docs | Eliminates parse failures; ~5% retry token reduction | All 5 | Not implemented | |
| G2 | **Motion-based chunk pre-screening** — mean absolute frame-diff within ROI per chunk; skip if below threshold | [DyToK arXiv 2512.06866] | 30–50% fewer Phase 1 API calls on idle periods | All 5 | Not implemented | |
